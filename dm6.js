      ...
      states: {
        ...
        GetName: {
          ...
          on: {
            ...
            //   actions: [assign({
            //     meetingWithName:({ event }) => event.nluvalue[0].utterance.toLowerCase(),
            //     // meetingWithName:({ event }) => event.nluValue[0].utterance.confidence.toLowerCase(),
            //     // meetingWithName:({ event }) => event.nluValue[0].utterance.intent.toLowerCase(),
            //   }),
            //   ({ event }) => console.log(event.nluValue[0].confidence),
            //   ({ event }) => console.log(event.nluValue[1].intent),
            ...
            actions: [
              assign({
                meetingWithName: ({ event }) => {
                  if (event && event.value && event.value.length > 0) {
                    return event.value[0].utterance.toLowerCase();
                    const topIntent = state.event.nluValue.topIntent;
                    console.log("Top Intent:", topIntent);
                  } else {
                    console.log("Utterance data not available.");
              ...
              ({ event }) => {
                if (event && event.value && event.value.length > 0) {
                  console.log("Intent:", event.value[0].intent);
                  console.log("Confidence:", event.value[0].confidence);
                  
                } else {
                  console.log("NLU data not available.");
                }
              },
            ],
